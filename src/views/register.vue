...
import {NButton, NFlex, NForm, NFormItem, NGi, NGrid, NInput, NLayout, NSpin, useMessage} from "naive-ui";
import {ref} from "vue";
		...
		Service.Users.register(model.value)
			.then(res => {
				...
				if (data?.code !== ResponseCode.SUCCESS) {
					...
					message.success("REGISTER_SUCCESS: 注册成功，验证链接已经发送到您的邮箱", messageOptions)
					// todo 重定向到登录界面 （3秒后）
				}
			})
			.catch(err => {
...
<template>
	...
	<n-layout position="absolute">
		<n-flex justify="center">
			<n-flex class="w-120 p-8 m-t-16 b-rd-4" justify="center">
				<div class="w-100%">
