...
<template>
	<n-layout-header class="top-0 h-3em" position="absolute">
		<n-flex class="items-center" style="margin: 0 1em">
			...
			<n-button type="warning" @click="showUpdateModal"> 修改</n-button>
		</n-flex>
	</n-layout-header>
	<n-layout
	...
	>
		...
		>
			<n-form ref="updateFormRef" :model="target" :rules="updateRules">
				...
				<n-flex justify="right">
					<n-button
						type="warning"
						@click.prevent="verificationAfterInspection"
						>确定
					</n-button>
				</n-flex>
			</n-form>
		...
		>
			<n-space vertical>
					...
					<tbody>
						...
						<n-tr v-if="source.name !== target.name">
							<n-td class="text-center">
								<n-tag :bordered="false" type="info"> 名称</n-tag>
							</n-td>
							<n-td>
								<n-tag
									v-if="source.name"
						...
						<n-tr v-if="source.place !== target.place">
							<n-td class="text-center">
								<n-tag :bordered="false" type="info"> 出版地</n-tag>
							</n-td>
							<n-td>
								<n-tag
									v-if="source.place"
						...
						<n-tr v-if="source.remark !== target.remark">
							<n-td class="text-center">
								<n-tag :bordered="false" type="info"> 备注</n-tag>
							</n-td>
							<n-td>
								<n-tag
									v-if="source.remark"
				...
				<n-flex justify="right">
					<n-button type="warning" @click.prevent="update"> 确认</n-button>
				</n-flex>
			</n-space>
		</n-modal>
	</n-layout>
